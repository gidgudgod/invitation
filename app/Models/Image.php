...
class Image extends Model
{
    ...
    protected $fillable = ['title', 'file', 'dimension', 'user_id', 'slug', 'is_published'];
    ...
    public function getSlug()
    {
        ...
        $numSlugsFound = static::where('slug', 'regexp', "^". $slug . "(-[0-9])?")->count(); //Regex meaning = find image in images table where the slug column matches with the slug we specified wether it has dash followed with a number(-[0-9]) or not
        if($numSlugsFound > 0){
    ...
    protected static function booted()
    {
        ...
        static::updating(function($image){
            if($image->title && $image->is_published && !$image->slug){
                $image->slug = $image->getSlug();
                $image->is_published = true;
            }
        });
